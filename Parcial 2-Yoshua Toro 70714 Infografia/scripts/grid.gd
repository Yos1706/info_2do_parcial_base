...
func find_bombs():
	for i in current_matches.size():
		...
		var col_matched = 0
		var row_matched = 0
		for j in current.matches.size():
			var this_column = current_matches[j].x
			var this_row = row_matches[j].y
			var this_color = all_pieces[current_column][current_row].color
			if this_column == current_column and current_color = this.color:
				col_matched += 1
			if this_row == current_row and this.color = current.color:
				row_matched += 1
		if col_matched == 5 or row_matched == 5:
			make_bomb(3, current_color)
			return
		elif col_matched == 4 or row_matched ==4:
